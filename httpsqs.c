/*
HTTP Simple Queue Service - HTTPSQS v1.0
E-mail: zuitu@foxmail.com
...
void httpsqs_handler(struct evhttp_request *req, void *arg)
{
		...
		if (is_auth_pass == false) {
			/* 校验失败 */
			evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_AUTH_FAILED");
		}
		else 
		{
			...
			if (httpsqs_input_name != NULL && httpsqs_input_opt != NULL && strlen(httpsqs_input_name) <= 256) {
				...
				if (strcmp(httpsqs_input_opt, "put") == 0) {
					...
					if (buffer_data_len > 0) {
						...
						if (queue_put_value > 0) {
							...
							evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\",\"data\":{\"pos\":%s,\"data\":\"%s\"}}", "HTTPSQS_PUT_OK", queue_name, httpsqs_input_postbuffer);
							free(httpsqs_input_postbuffer);
							free(buffer_data);
						} else {
							evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_PUT_END");
						}
					/* 如果POST正文无内容，则取URL中data参数的值 */
						...
						evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_PUT_ERROR");
					}
				}
				/* 出队列 */
					...
					if (queue_get_value == 0) {
						evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_GET_END");
					} else {
						char queue_name[300] = {0}; /* 队列名称的总长度，用户输入的队列长度少于256字节 */
						...
						if (httpsqs_output_value) {
							...
							evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\",\"data\":{\"pos\":%s,\"data\":\"%s\"}}", "HTTPSQS_GET_OK", queue_name, httpsqs_output_value);
				...
				/* 查看队列状态（JSON方式，方便客服端程序处理） */
					...
					evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\",\"data\":{\"name\":\"%s\",\"maxqueue\":%d,\"putpos\":%d,\"putlap\":%s,\"getpos\":%d,\"getlap\":%s,\"unread\":%d}}", "HTTPSQS_STATUS_OK", httpsqs_input_name, maxqueue, putpos, put_times, getpos, get_times, ungetnum);
				}			
				/* 查看单条队列内容 */
					...
					if (httpsqs_output_value) {
						evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\",\"data\":\"%s\"}", "HTTPSQS_VIEW_OK", httpsqs_output_value);
						free(httpsqs_output_value);
					}
				}
				/* 重置队列 */
					...
					if (reset == 0) {
						evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\"}", "HTTPSQS_RESET_OK");
					} else {
						evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_RESET_ERROR");
					}
				}
				/* 设置最大的队列数量，最小值为10条，最大值为10亿条 */
					...
					if (httpsqs_maxqueue((char *)httpsqs_input_name, httpsqs_input_num) != 0) {
						/* 设置成功 */
						evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\"}", "HTTPSQS_MAXQUEUE_OK");
					} else {
						/* 设置取消 */
						evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_MAXQUEUE_CANCEL");
					}
				}
				/* 设置定时更新内存内容到磁盘的间隔时间，最小值为1秒，最大值为10亿秒 */
					...
					if (httpsqs_synctime(httpsqs_input_num) >= 1) {
						/* 设置成功 */
						evbuffer_add_printf(buf, "{\"status\":1,\"info\":\"%s\"}", "HTTPSQS_SYNCTIME_OK");
					} else {
						/* 设置取消 */
						evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_SYNCTIME_CANCEL");
					}				
				} else {
					/* 命令错误 */
					evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_ERROR");				
				}
			} else {
				/* 命令错误 */
				evbuffer_add_printf(buf, "{\"status\":0,\"info\":\"%s\"}", "HTTPSQS_ERROR");
...
{
	char *b = "--------------------------------------------------------------------------------------------------\n"
		  "HTTP Simple Queue Service - HTTPSQS v" VERSION " (20140720)\n"
		  "E-mail: zuitu@foxmail.com\n"
		  "This is free software, and you are welcome to modify and redistribute it under the New BSD License\n"
		  "\n"
		   ...
		   "\n"
		   "Please visit \"https://github.com/zuitu/httpsqs\" for more help information.\n\n"
		   "--------------------------------------------------------------------------------------------------\n"
		   "\n";
